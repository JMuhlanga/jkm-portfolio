    ...
    <section id="projects" className="py-20 px-4 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {projects.map((project, index) => (
          <div key={index} className="group bg-white dark:bg-slate-800 p-8 rounded-lg transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-cyan-900/10 border border-slate-200 dark:border-transparent">
            <div className="flex justify-between items-start mb-6">
              <div className="text-cyan-600 dark:text-cyan-400 p-2 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                {project.icon}
              </div>
              <div className="flex gap-4">
                <a href="#" className="text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">
                  <Github size={20} />
                </a>
                <a href="#" className="text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">
